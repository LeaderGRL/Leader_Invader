...
fn render(topology: &Topology, trace: &MatchTrace, config: RenderConfig) -> String {
    ...
    let mut out = String::with_capacity(360_000);
...
mod tests {
    ...
    fn sampler_preserves_each_transition_class() {
        ...
        let mut rendered_execute = false;
        for event in &trace.micro_addresses {
            if sampler.take(event.source, event.address) {
    ...
    #[test]
    fn complete_match_contains_cmpi_five_row_progression() {
        let trace = Machine::run_match("microcode-cmpi", 5000);
        let base = execute_address(op::CMPI).unwrap();
        for step in 0..5 {
            assert!(
                trace.micro_addresses.iter().any(|event| event.opcode == op::CMPI && event.address == base + step),
                "missing CMPI execute row {step}"
            );
        }
    }
