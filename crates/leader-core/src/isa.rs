...
pub struct Cpu {
    ...
    address_lo: u8,
    address_hi: u8,
    condition_latch: bool,
    pc_selected: Option<u16>,
    flags: Flags,
    halted: bool,
}
...
impl Cpu {
        ...
        match micro.operation {
            ...
            MicroOp::LoadImmediate => {
                ...
                if self.active_control().alu_enable {
                    self.latch_flags(Flags { zero: value == 0, carry: false, less: false });
    ...
    fn select_pc_target(&mut self, target: Option<u16>) {
        if self.internal_enabled(internal::PC_SELECT) { self.pc_selected = target; }
    }

    fn commit_selected_pc<B: Bus>(&mut self, bus: &mut B, source: PcSource, control: &'static str) {
        let word = self.active_control();
        if !word.pc_load || !word.has_internal(internal::ARCH_COMMIT) { return; }
        let Some(target) = self.pc_selected else { return; };
        let before = self.pc;
        self.pc = target;
    ...
    fn latched_alu_trace(&self) -> Option<AluTrace> {
        ...
        Some(match operation {
            ...
            AluOp::And => logic_trace(operation, self.alu_a, self.alu_b, self.alu_a & self.alu_b),
            AluOp::Or => logic_trace(operation, self.alu_a, self.alu_b, self.alu_a | self.alu_b),
            AluOp::Xor => logic_trace(operation, self.alu_a, self.alu_b, self.alu_a ^ self.alu_b),
        })
    }

    fn propagate_latched_alu<B: Bus>(&mut self, bus: &mut B, pc: u16, control: &'static str, compare: bool) -> Option<AluTrace> {
        if !self.active_control().alu_enable { return None; }
        let trace = self.latched_alu_trace()?;
        if compare {
            self.latch_compare_flags(trace);
        } else if trace.op == AluOp::Pass || matches!(trace.op, AluOp::And | AluOp::Or | AluOp::Xor) {
            self.latch_flags(Flags { zero: trace.result == 0, carry: false, less: false });
        } else {
    ...
    fn increment_pc_controlled<B: Bus>(&mut self, bus: &mut B) {
        if !self.internal_enabled(internal::PC_INC) { return; }
    ...
    fn read_bus_enabled(&self) -> bool {
        ...
        control.mem_read && control.has_internal(internal::MDR_LOAD) && control.has_internal(internal::BUS_DATA_ENABLE)
    }
    ...
    fn fetch_opcode<B: Bus>(&mut self, bus: &mut B) -> u8 {
        ...
        if self.read_bus_enabled() { let value = bus.fetch8(self.mar); self.latch_mdr(value); }
    ...
    fn read_memory<B: Bus>(&mut self, bus: &mut B, pc: u16, address: u16, control: &'static str) -> u8 {
        ...
        let transition = self.micro.advance();
        self.publish_microaddress(bus, transition, control);
        if self.read_bus_enabled() { let value = bus.read8(pc, self.mar); self.latch_mdr(value); }
    ...
    fn write_memory<B: Bus>(&mut self, bus: &mut B, pc: u16, address: u16, value: u8, control: &'static str) {
        ...
        let transition = self.micro.advance();
        self.publish_microaddress(bus, transition, control);
        if self.write_bus_commit_enabled() { bus.write8(pc, self.mar, self.mdr); }
    ...
    fn write_reg<B: Bus>(&mut self, bus: &mut B, pc: u16, reg: Reg, value: u8, control: &'static str) {
        if !self.active_control().reg_write { return; }
    ...
    fn load_memory<B: Bus>(&mut self, bus: &mut B, pc: u16, opcode: u8, control: &'static str) -> StepOutcome {
        ...
        self.latch_address_lo(lo);
        self.advance_execute(bus, "LD_ADDR_HI");
        let hi = self.next8(bus);
        self.latch_address_hi(hi);
        self.advance_execute(bus, "LD_READ");
        let value = self.read_memory(bus, pc, self.address_latch(), "LD");
        self.advance_execute(bus, "LD_COMMIT");
        self.write_reg(bus, pc, reg, value, control);
        self.latch_flags(Flags { zero: value == 0, carry: self.flags.carry, less: false });
        StepOutcome::Continue
    }
    ...
    fn store_memory<B: Bus>(&mut self, bus: &mut B, pc: u16, opcode: u8) -> StepOutcome {
        ...
        self.write_memory(bus, pc, self.address_latch(), value, "ST");
        self.advance_execute(bus, "ST_COMMIT");
        StepOutcome::Continue
    }

    fn control_transfer<B: Bus>(&mut self, bus: &mut B, pc: u16, condition: bool, source: PcSource, control: &'static str) -> StepOutcome {
        let lo = self.next8(bus);
        self.latch_address_lo(lo);
        ...
        self.latch_condition(condition);
        self.advance_execute(bus, "PC_SELECT");
        let selected = self.condition_latch.then_some(self.address_latch());
        self.select_pc_target(selected);
        self.advance_execute(bus, "PC_COMMIT");
        self.commit_selected_pc(bus, source, control);
        bus.trace_control(pc, control);
        StepOutcome::Continue
    }

    fn call_subroutine<B: Bus>(&mut self, bus: &mut B, pc: u16, control: &'static str) -> StepOutcome {
        let lo = self.next8(bus);
        self.latch_address_lo(lo);
        ...
        self.select_pc_target(Some(self.address_latch()));
        self.commit_selected_pc(bus, PcSource::Call, control);
        bus.trace_control(pc, control);
        StepOutcome::Continue
    }

    fn return_subroutine<B: Bus>(&mut self, bus: &mut B, pc: u16, control: &'static str) -> StepOutcome {
        ...
        self.select_pc_target(Some(target));
        self.advance_execute(bus, "RET_PC_COMMIT");
        self.commit_selected_pc(bus, PcSource::Return, control);
        bus.trace_control(pc, control);
        StepOutcome::Continue
    }
    ...
    fn binary_register_alu<B: Bus>(&mut self, bus: &mut B, pc: u16, opcode: u8, operation: AluOp, control: &'static str, compare: bool) -> StepOutcome {
        ...
        let Some(trace) = self.propagate_latched_alu(bus, pc, control, compare) else { return self.fault(pc, opcode); };
        self.advance_execute(bus, "ALU_COMMIT");
        if !compare { self.write_reg(bus, pc, lhs_reg, trace.result, control); }
        StepOutcome::Continue
    }
    ...
    fn immediate_arithmetic<B: Bus>(&mut self, bus: &mut B, pc: u16, opcode: u8, operation: AluOp, control: &'static str) -> StepOutcome {
        ...
        if opcode == op::ADDI {
            ...
            if !self.active_control().alu_enable { return self.fault(pc, opcode); }
    ...
    fn compare_immediate<B: Bus>(&mut self, bus: &mut B, pc: u16, opcode: u8, control: &'static str) -> StepOutcome {
        ...
        self.advance_execute(bus, "ALU_PROPAGATE");
        let Some(_trace) = self.propagate_latched_alu(bus, pc, control, true) else { return self.fault(pc, opcode); };
        self.advance_execute(bus, "ALU_COMMIT");
        StepOutcome::Continue
    }

    fn latch_arithmetic_flags(&mut self, trace: AluTrace) {
        self.latch_flags(Flags { zero: trace.result == 0, carry: trace.final_carry(), less: !trace.final_carry() && matches!(trace.op, AluOp::Sub) });
    }

    fn latch_compare_flags(&mut self, trace: AluTrace) {
        self.latch_flags(Flags { zero: trace.result == 0, carry: trace.final_carry(), less: !trace.final_carry() });
    }
    ...
    fn push<B: Bus>(&mut self, bus: &mut B, pc: u16, value: u8) {
        if !self.active_control().stack_enable { return; }
    ...
    fn pop<B: Bus>(&mut self, bus: &mut B, pc: u16) -> u8 {
        if !self.active_control().stack_enable { return 0; }
...
#[must_use]
pub const fn mnemonic(value: u8) -> &'static str { match decode_microcode(value) { Some(instruction) => instruction.mnemonic, None => "FAULT" } }
...
mod tests {
    ...
    impl Default for TestBus {
        fn default() -> Self {
            Self { memory: vec![0; 65536], exact_alu: vec![], writes: vec![], pc_increments: vec![], pc_loads: vec![], cycles: vec![], micro_addresses: vec![] }
        }
    }
    ...
    impl Bus for TestBus {
        ...
        fn trace_microaddress(&mut self, transition: MicroAddressTransition, _opcode: u8, _bits: u8, _label: &'static str) { self.micro_addresses.push(transition); }
    ...
    fn assert_five_execute_rows(cpu: &Cpu, bus: &TestBus, opcode: u8) {
        ...
        for step in 0..5 {
            assert!(bus.micro_addresses.iter().any(|transition| transition.after == base + step), "missing opcode {opcode:02X} execute row {step}");
        }
    ...
    fn flags_latch_requires_flags_load_on_selected_row() {
        ...
        cpu.latch_flags(Flags { zero: true, carry: true, less: true });
        assert_eq!(cpu.flags, Flags::default());
        cpu.micro.dispatch(base + 3);
        cpu.latch_flags(Flags { zero: true, carry: true, less: false });
        assert_eq!(cpu.flags, Flags { zero: true, carry: true, less: false });
        cpu.ir = op::LDI;
        let ldi = execute_address(op::LDI).unwrap();
        cpu.flags = Flags::default();
        cpu.micro.dispatch(ldi + 2);
        cpu.latch_flags(Flags { zero: true, carry: false, less: false });
    ...
    fn selected_alu_operation_is_not_the_callers_local_operation_after_latching() {
        ...
        cpu.micro.dispatch(base + 3);
        assert_eq!(cpu.latched_alu_trace().unwrap().result, 5);
        cpu.micro.dispatch(base + 2);
        cpu.latch_alu_op(AluOp::Add);
        cpu.micro.dispatch(base + 3);
        assert_eq!(cpu.latched_alu_trace().unwrap().result, 13);
    }

    #[test]
    fn address_condition_and_pc_selection_require_their_physical_rows() {
        let mut bus = TestBus::default();
        let mut cpu = Cpu::default();
        cpu.ir = op::JZ;
        let base = execute_address(op::JZ).unwrap();
        cpu.address_lo = 0xAA;
        cpu.address_hi = 0xBB;
        cpu.micro.dispatch(base + 1);
        cpu.latch_address_lo(0x34);
        assert_eq!(cpu.address_lo, 0xAA);
        cpu.micro.dispatch(base);
        cpu.latch_address_lo(0x34);
        assert_eq!(cpu.address_lo, 0x34);
        cpu.micro.dispatch(base);
        cpu.latch_address_hi(0x12);
        assert_eq!(cpu.address_hi, 0xBB);
        cpu.micro.dispatch(base + 1);
        cpu.latch_address_hi(0x12);
        assert_eq!(cpu.address_latch(), 0x1234);
        cpu.micro.dispatch(base + 1);
        cpu.latch_condition(true);
        assert!(!cpu.condition_latch);
        cpu.micro.dispatch(base + 2);
        cpu.latch_condition(true);
        assert!(cpu.condition_latch);
        cpu.micro.dispatch(base + 2);
        cpu.select_pc_target(Some(cpu.address_latch()));
        assert_eq!(cpu.pc_selected, None);
        cpu.micro.dispatch(base + 3);
        cpu.select_pc_target(Some(cpu.address_latch()));
        assert_eq!(cpu.pc_selected, Some(0x1234));
        cpu.pc = 7;
        cpu.commit_selected_pc(&mut bus, PcSource::Branch, "JZ");
        assert_eq!(cpu.pc, 7);
        cpu.micro.dispatch(base + 4);
        cpu.commit_selected_pc(&mut bus, PcSource::Branch, "JZ");
        assert_eq!(cpu.pc, 0x1234);
        assert_eq!(bus.pc_loads.len(), 1);
    }

    #[test]
    fn not_taken_branch_clears_stale_pc_selection() {
        let mut cpu = Cpu::default();
        cpu.ir = op::JZ;
        let base = execute_address(op::JZ).unwrap();
        cpu.pc_selected = Some(0xBEEF);
        cpu.address_lo = 0x34;
        cpu.address_hi = 0x12;
        cpu.condition_latch = false;
        cpu.micro.dispatch(base + 3);
        cpu.select_pc_target(cpu.condition_latch.then_some(cpu.address_latch()));
        assert_eq!(cpu.pc_selected, None);
    }
    ...
    fn control_rom_gates_register_pc_and_stack_effects() {
        ...
        cpu.ir = op::JMP;
        cpu.pc = 3;
        let jump = execute_address(op::JMP).unwrap();
        cpu.micro.dispatch(jump + 2);
        cpu.select_pc_target(Some(9));
        assert_eq!(cpu.pc_selected, None);
        cpu.micro.dispatch(jump + 3);
        cpu.select_pc_target(Some(9));
        assert_eq!(cpu.pc_selected, Some(9));
        cpu.commit_selected_pc(&mut bus, PcSource::Jump, "JMP");
        assert_eq!(cpu.pc(), 3);
        cpu.micro.dispatch(jump + 4);
        cpu.commit_selected_pc(&mut bus, PcSource::Jump, "JMP");
        assert_eq!(cpu.pc(), 9);
        cpu.ir = op::CALL;
        let initial = cpu.sp();
    ...
    fn basic_fetch_and_existing_three_row_ops_still_work() {
        let mut bus = TestBus::default();
        bus.memory[..7].copy_from_slice(&[op::LDI, 0, 4, op::ADDI, 0, 6, op::HALT]);
        let mut cpu = Cpu::default();
