...
pub fn apply_visual_layout(topology: &mut Topology) {
    ...
    inject_microaddress_bits(topology);
...
fn inject_decoder_lines(topology: &mut Topology) {
    ...
    // Keep the one-hot banks left of the dedicated µADDR bit column.
    for bank in ['A', 'B'] {
        ...
        for line in 0..16 {
            ...
            topology.nodes.push(Node {
                ...
                bounds: Rect::new(1260.0 + col * 65.0, base_y + row * 48.0, 58.0, 34.0),
            });
...
fn inject_microaddress_bits(topology: &mut Topology) {
    if topology.node("microAddrBit0").is_some() {
        return;
    }
    ...
    for bit in 0..8 {
        ...
        let id = format!("microAddrBit{bit}");
        topology.nodes.push(Node {
            id: id.clone(),
            title: format!("µA{bit}"),
            kind: "DFF".to_owned(),
            group: "decode".to_owned(),
            bounds: Rect::new(1780.0 + col * 62.0, 520.0 + row * 46.0, 48.0, 30.0),
        });
        topology.links.push(Link {
            id: format!("microaddr-bit-{bit}"),
            from: "microAddr".to_owned(),
            to: id.clone(),
            signal: SignalKind::Control,
            label: format!("µA{bit}"),
        });
        topology.links.push(Link {
            id: format!("microaddr-rom-{bit}"),
            from: id,
            to: "microRom".to_owned(),
            signal: SignalKind::Control,
            label: "µADDR".to_owned(),
        });
    }
}
...
fn inject_stack_pointer(topology: &mut Topology) {
    if topology.node("spBit0").is_some() {
        return;
    }
    ...
    topology.links.extend([
        Link {
            id: "sp-dec-in".to_owned(),
            from: "spBit0".to_owned(),
            to: "spDec".to_owned(),
            signal: SignalKind::Data,
            label: "SP".to_owned(),
        },
        Link {
            id: "sp-borrow".to_owned(),
            from: "spDec".to_owned(),
            to: "spBorrow".to_owned(),
            signal: SignalKind::Carry,
            label: "B8".to_owned(),
        },
        Link {
            id: "sp-inc-in".to_owned(),
            from: "spBit8".to_owned(),
            to: "spInc".to_owned(),
            signal: SignalKind::Data,
            label: "SP".to_owned(),
        },
        Link {
            id: "sp-address".to_owned(),
            from: "spInc".to_owned(),
            to: "addrBuf".to_owned(),
            signal: SignalKind::Address,
            label: "STACK ADDR".to_owned(),
        },
    ]);
}
...
mod tests {
    ...
    #[test]
    fn microaddress_register_exposes_all_eight_bits() {
        let mut topology = crate::topology::build_topology();
        apply_visual_layout(&mut topology);
        for bit in 0..8 {
    ...
    fn stack_pointer_has_sixteen_bits_and_ripple_nodes() {
        ...
        apply_visual_layout(&mut topology);
        for bit in 0..16 {
            assert!(topology.node(&format!("spBit{bit}")).is_some());
        }
        for id in ["spDec", "spBorrow", "spInc"] {
            assert!(topology.node(id).is_some());
        }
    }
}
