...
impl ControlWord {
    ...
    const fn with_internal(mut self, bits: u16) -> Self { self.internal = bits; self }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum MicroOp { Nop, LoadImmediate, LoadMemory, StoreMemory, Move, Add, AddImmediate, SubImmediate, AndImmediate, OrImmediate, XorImmediate, Increment, Decrement, Compare, CompareImmediate, Jump, JumpZero, JumpNotZero, JumpLess, JumpGreaterEqual, JumpCarry, Call, Return, WaitVBlank, Halt }
...
impl MicroAddressSource { #[must_use] pub const fn as_str(self) -> &'static str { match self { Self::FetchStart => "fetch_start", Self::Sequential => "sequential", Self::Dispatch => "dispatch", Self::RoutineCall => "routine_call", Self::RoutineReturn => "routine_return" } } }
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl ExecuteRowKind { #[must_use] pub const fn as_str(self) -> &'static str { match self { Self::Operand => "operand", Self::Address => "address", Self::Condition => "condition", Self::PcSelect => "pc_select", Self::AluSelect => "alu_select", Self::Propagate => "propagate", Self::Memory => "memory", Self::Stack => "stack", Self::Commit => "commit", Self::Idle => "idle" } } }
...
pub const fn decode(opcode: u8) -> Option<MicroInstruction> {
    let (mnemonic, operation, control) = match opcode {
        op::NOP=>("NOP",MicroOp::Nop,NONE), op::LDI=>("LDI",MicroOp::LoadImmediate,REG_ALU), op::LD=>("LD",MicroOp::LoadMemory,LOAD), op::ST=>("ST",MicroOp::StoreMemory,STORE), op::MOV=>("MOV",MicroOp::Move,REG_ALU), op::ADD=>("ADD",MicroOp::Add,REG_ALU), op::ADDI=>("ADDI",MicroOp::AddImmediate,REG_ALU), op::SUBI=>("SUBI",MicroOp::SubImmediate,REG_ALU), op::ANDI=>("ANDI",MicroOp::AndImmediate,REG_ALU), op::ORI=>("ORI",MicroOp::OrImmediate,REG_ALU), op::XORI=>("XORI",MicroOp::XorImmediate,REG_ALU), op::INC=>("INC",MicroOp::Increment,REG_ALU), op::DEC=>("DEC",MicroOp::Decrement,REG_ALU), op::CMP=>("CMP",MicroOp::Compare,ALU_ONLY), op::CMPI=>("CMPI",MicroOp::CompareImmediate,ALU_ONLY), op::JMP=>("JMP",MicroOp::Jump,PC_LOAD), op::JZ=>("JZ",MicroOp::JumpZero,PC_LOAD), op::JNZ=>("JNZ",MicroOp::JumpNotZero,PC_LOAD), op::JLT=>("JLT",MicroOp::JumpLess,PC_LOAD), op::JGE=>("JGE",MicroOp::JumpGreaterEqual,PC_LOAD), op::JC=>("JC",MicroOp::JumpCarry,PC_LOAD), op::CALL=>("CALL",MicroOp::Call,CALL), op::RET=>("RET",MicroOp::Return,RET), op::WAIT_VBLANK=>("WAIT_VBLANK",MicroOp::WaitVBlank,WAIT), op::HALT=>("HALT",MicroOp::Halt,HALT), _=>return None,
...
#[must_use] pub const fn opcode_slot(opcode:u8)->Option<u8>{Some(match opcode{op::NOP=>0,op::LDI=>1,op::LD=>2,op::ST=>3,op::MOV=>4,op::ADD=>5,op::ADDI=>6,op::SUBI=>7,op::ANDI=>8,op::ORI=>9,op::XORI=>10,op::INC=>11,op::DEC=>12,op::CMP=>13,op::CMPI=>14,op::JMP=>15,op::JZ=>16,op::JNZ=>17,op::JLT=>18,op::JGE=>19,op::JC=>20,op::CALL=>21,op::RET=>22,op::WAIT_VBLANK=>23,op::HALT=>24,_=>return None})}
...
#[must_use] pub const fn execute_control_step(opcode:u8)->u8{if is_five_row_alu(opcode)||is_five_row_memory(opcode)||is_five_row_branch(opcode)||is_five_row_stack(opcode){4}else{match opcode{op::LDI|op::ADDI=>2,_=>0}}}
...
const fn execute_internal(opcode:u8,step:u8)->u16{
    use internal::*;
    if is_five_row_alu(opcode){return match step{0=>OPERAND_A_LOAD|REG_SELECT,1=>if matches!(opcode,op::MOV|op::ADD|op::CMP){OPERAND_B_LOAD|REG_SELECT}else{OPERAND_B_LOAD},2=>ALU_OP_LOAD,3=>FLAGS_LOAD,4=>ARCH_COMMIT,_=>0};}
    if matches!(opcode,op::LD|op::ST){return match step{0=>if opcode==op::LD{OPERAND_A_LOAD|REG_SELECT}else{ADDR_LO_LOAD},1=>if opcode==op::LD{ADDR_LO_LOAD}else{ADDR_HI_LOAD},2=>if opcode==op::LD{ADDR_HI_LOAD}else{OPERAND_A_LOAD|REG_SELECT},3=>BUS_ADDRESS_ENABLE|BUS_DATA_ENABLE,4=>if opcode==op::LD{FLAGS_LOAD|ARCH_COMMIT}else{ARCH_COMMIT},_=>0};}
    if is_five_row_branch(opcode){return match step{0=>ADDR_LO_LOAD,1=>ADDR_HI_LOAD,2=>CONDITION_LOAD,3=>PC_SELECT,4=>ARCH_COMMIT,_=>0};}
    if opcode==op::CALL{return match step{0=>ADDR_LO_LOAD,1=>ADDR_HI_LOAD,2|3=>BUS_ADDRESS_ENABLE|BUS_DATA_ENABLE,4=>PC_SELECT|ARCH_COMMIT,_=>0};}
    if opcode==op::RET{return match step{0=>BUS_ADDRESS_ENABLE|BUS_DATA_ENABLE|ADDR_LO_LOAD,1=>BUS_ADDRESS_ENABLE|BUS_DATA_ENABLE|ADDR_HI_LOAD,2|3=>PC_SELECT,4=>ARCH_COMMIT,_=>0};}
    match opcode{op::LDI|op::ADDI=>match step{0=>OPERAND_A_LOAD|REG_SELECT,1=>OPERAND_B_LOAD|ALU_OP_LOAD,2=>FLAGS_LOAD|ARCH_COMMIT,_=>0},_=>if step==0{ARCH_COMMIT}else{0}}
}

#[must_use]
pub const fn execute_row_control(opcode:u8,step:u8)->ControlWord{
    let external=if is_five_row_alu(opcode){match step{2|3=>ALU_ONLY,4=>match opcode{op::CMP|op::CMPI=>NONE,_=>REG_WRITE},_=>NONE}}else if is_five_row_memory(opcode){match(opcode,step){(op::LD,4)=>REG_WRITE,_=>NONE}}else if is_five_row_branch(opcode){if step==4{PC_LOAD}else{NONE}}else if opcode==op::CALL{match step{2|3=>STACK_ONLY,4=>PC_LOAD,_=>NONE}}else if opcode==op::RET{match step{0|1=>STACK_ONLY,4=>PC_LOAD,_=>NONE}}else if step==execute_control_step(opcode){control_word(opcode)}else{NONE};external.with_internal(execute_internal(opcode,step))
}
...
mod tests{
    ...
    #[test]fn execute_blocks_fill_80_through_fc_without_overlap(){let mut seen=[false;256];for opcode in OPCODES{let base=execute_address(opcode).unwrap();for step in 0..5{let address=execute_step_address(opcode,step).unwrap();assert_eq!(address,base+step);assert!(!seen[address as usize]);seen[address as usize]=true;}}assert_eq!(execute_step_address(op::HALT,4),Some(uaddr::EXEC_LAST));}
    #[test]fn physical_word_is_really_twenty_four_bits_wide(){let fetch0=control_word_at(uaddr::FETCH_T0,op::NOP);let fetch1=control_word_at(uaddr::FETCH_T1,op::NOP);let fetch2=control_word_at(uaddr::FETCH_T2,op::NOP);assert!(fetch0.has_internal(internal::MAR_LOAD));assert!(fetch0.has_internal(internal::BUS_ADDRESS_ENABLE));assert!(fetch1.mem_read);assert!(fetch1.has_internal(internal::MDR_LOAD));assert!(fetch1.has_internal(internal::PC_INC));assert!(fetch2.has_internal(internal::IR_LOAD));assert_ne!(fetch0.bits24()>>8,0);}
    #[test]fn five_row_alu_exposes_internal_latch_path(){let base=execute_address(op::ADD).unwrap();assert!(control_word_at(base,op::ADD).has_internal(internal::OPERAND_A_LOAD));assert!(control_word_at(base+1,op::ADD).has_internal(internal::OPERAND_B_LOAD));assert!(control_word_at(base+1,op::ADD).has_internal(internal::REG_SELECT));assert!(control_word_at(base+2,op::ADD).has_internal(internal::ALU_OP_LOAD));assert!(control_word_at(base+3,op::ADD).has_internal(internal::FLAGS_LOAD));assert!(control_word_at(base+4,op::ADD).has_internal(internal::ARCH_COMMIT));assert!(control_word_at(base+4,op::ADD).reg_write);}
    #[test]fn register_select_is_only_asserted_for_register_encoded_operands(){for opcode in[op::MOV,op::ADD,op::CMP]{let base=execute_address(opcode).unwrap();assert!(control_word_at(base+1,opcode).has_internal(internal::REG_SELECT));}for opcode in[op::SUBI,op::ANDI,op::ORI,op::XORI,op::INC,op::DEC,op::CMPI]{let base=execute_address(opcode).unwrap();assert!(control_word_at(base+1,opcode).has_internal(internal::OPERAND_B_LOAD));assert!(!control_word_at(base+1,opcode).has_internal(internal::REG_SELECT));}}
    #[test]fn branch_and_stack_rows_expose_internal_selects(){let branch=execute_address(op::JZ).unwrap();assert!(control_word_at(branch,op::JZ).has_internal(internal::ADDR_LO_LOAD));assert!(control_word_at(branch+1,op::JZ).has_internal(internal::ADDR_HI_LOAD));assert!(control_word_at(branch+2,op::JZ).has_internal(internal::CONDITION_LOAD));assert!(control_word_at(branch+3,op::JZ).has_internal(internal::PC_SELECT));assert!(control_word_at(branch+4,op::JZ).pc_load);let call=execute_address(op::CALL).unwrap();assert!(control_word_at(call+2,op::CALL).stack_enable);assert!(control_word_at(call+2,op::CALL).has_internal(internal::BUS_DATA_ENABLE));let ret=execute_address(op::RET).unwrap();assert!(control_word_at(ret,op::RET).has_internal(internal::ADDR_LO_LOAD));assert!(control_word_at(ret+1,op::RET).has_internal(internal::ADDR_HI_LOAD));assert!(control_word_at(ret+3,op::RET).has_internal(internal::PC_SELECT));assert!(control_word_at(ret+4,op::RET).pc_load);}
    #[test]fn compact_ldi_addi_rows_expose_full_latched_alu_path(){for opcode in[op::LDI,op::ADDI]{assert_eq!(execute_row_kind(opcode,0),Some(ExecuteRowKind::Operand));assert_eq!(execute_row_kind(opcode,1),Some(ExecuteRowKind::Operand));assert_eq!(execute_row_kind(opcode,2),Some(ExecuteRowKind::Commit));let base=execute_address(opcode).unwrap();let row0=control_word_at(base,opcode);let row1=control_word_at(base+1,opcode);let row2=control_word_at(base+2,opcode);assert!(row0.has_internal(internal::OPERAND_A_LOAD));assert!(row0.has_internal(internal::REG_SELECT));assert!(row1.has_internal(internal::OPERAND_B_LOAD));assert!(row1.has_internal(internal::ALU_OP_LOAD));assert!(!row1.has_internal(internal::REG_SELECT));assert!(row2.has_internal(internal::FLAGS_LOAD));assert!(row2.has_internal(internal::ARCH_COMMIT));assert!(row2.alu_enable);assert!(row2.reg_write);}let ld=execute_address(op::LD).unwrap();assert!(control_word_at(ld+4,op::LD).has_internal(internal::FLAGS_LOAD));}
    #[test]fn microsequencer_has_real_increment_dispatch_and_return_paths(){let mut seq=MicroSequencer::default();assert_eq!(seq.advance().after,uaddr::FETCH_T1);let exec=execute_address(op::ADDI).unwrap();assert_eq!(seq.dispatch(exec).after,exec);assert_eq!(seq.call(uaddr::OPERAND_T0).after,uaddr::OPERAND_T0);assert_eq!(seq.advance().after,uaddr::OPERAND_T1);assert_eq!(seq.advance().after,uaddr::OPERAND_T2);assert_eq!(seq.return_from_routine().after,exec);}
}
