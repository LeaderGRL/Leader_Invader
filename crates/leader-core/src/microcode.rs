...
impl ControlWord {
    ...
    #[must_use]
    pub const fn internal_bits(self) -> u16 {
        self.internal
    }

    #[must_use]
    pub const fn bits24(self) -> u32 {
        (self.bits() as u32) | ((self.internal as u32) << 8)
    }

    #[must_use]
    pub const fn has_internal(self, signal: u16) -> bool {
        self.internal & signal != 0
    }
...
const REG_WRITE: ControlWord =
    ControlWord::new(true, false, false, false, false, false, false, false);
const ALU_ONLY: ControlWord =
    ControlWord::new(false, true, false, false, false, false, false, false);
...
const PC_LOAD: ControlWord =
    ControlWord::new(false, false, false, false, true, false, false, false);
const STACK_ONLY: ControlWord =
    ControlWord::new(false, false, false, false, false, true, false, false);
...
#[must_use]
pub const fn is_five_row_memory(opcode: u8) -> bool {
    matches!(opcode, op::LD | op::ST)
}

#[must_use]
pub const fn is_five_row_branch(opcode: u8) -> bool {
    matches!(
        opcode,
        op::JMP | op::JZ | op::JNZ | op::JLT | op::JGE | op::JC
    )
}

#[must_use]
pub const fn is_five_row_stack(opcode: u8) -> bool {
    matches!(opcode, op::CALL | op::RET)
}

#[must_use]
pub const fn execute_control_step(opcode: u8) -> u8 {
    if is_five_row_alu(opcode)
        || is_five_row_memory(opcode)
        || is_five_row_branch(opcode)
        || is_five_row_stack(opcode)
    {
        4
    } else {
        match opcode {
...
pub const fn execute_row_kind(opcode: u8, step: u8) -> Option<ExecuteRowKind> {
    ...
    if is_five_row_branch(opcode) {
        return Some(match step {
            0 | 1 => ExecuteRowKind::Address,
            2 => ExecuteRowKind::Condition,
            3 => ExecuteRowKind::PcSelect,
            4 => ExecuteRowKind::Commit,
            _ => ExecuteRowKind::Idle,
        });
    }
    ...
    if opcode == op::RET {
        return Some(match step {
            0 | 1 => ExecuteRowKind::Stack,
            2 | 3 => ExecuteRowKind::PcSelect,
            4 => ExecuteRowKind::Commit,
            _ => ExecuteRowKind::Idle,
        });
    }
...
const fn execute_internal(opcode: u8, step: u8) -> u16 {
    ...
    if opcode == op::CALL {
        return match step {
            0 => ADDR_LO_LOAD,
            1 => ADDR_HI_LOAD,
            2 | 3 => BUS_ADDRESS_ENABLE | BUS_DATA_ENABLE,
            4 => PC_SELECT | ARCH_COMMIT,
            _ => 0,
        };
    }
    if opcode == op::RET {
        return match step {
            0 | 1 => BUS_ADDRESS_ENABLE | BUS_DATA_ENABLE,
            2 | 3 => PC_SELECT,
            4 => ARCH_COMMIT,
            _ => 0,
        };
    ...
    match opcode {
        op::LDI | op::ADDI => match step {
            0 => OPERAND_A_LOAD | REG_SELECT,
            1 => OPERAND_B_LOAD | ALU_OP_LOAD,
            2 => FLAGS_LOAD | ARCH_COMMIT,
            _ => 0,
        },
        _ => {
            if step == 0 {
...
pub const fn execute_row_control(opcode: u8, step: u8) -> ControlWord {
    ...
    } else if step == execute_control_step(opcode) {
        control_word(opcode)
    } else {
        NONE
    };
...
pub fn control_word_at(address: u8, opcode: u8) -> ControlWord {
    ...
    match address {
        ...
        uaddr::FETCH_T1 => ControlWord::new(false, false, true, false, false, false, false, false)
            .with_internal(MDR_LOAD | PC_INC | BUS_DATA_ENABLE),
        uaddr::FETCH_T2 => NONE.with_internal(IR_LOAD),
        uaddr::OPERAND_T0 => NONE.with_internal(MAR_LOAD | BUS_ADDRESS_ENABLE),
        uaddr::OPERAND_T1 => ControlWord::new(false, false, true, false, false, false, false, false)
            .with_internal(MDR_LOAD | PC_INC | BUS_DATA_ENABLE),
        uaddr::OPERAND_T2 => NONE,
        uaddr::READ_T0 => NONE.with_internal(MAR_LOAD | BUS_ADDRESS_ENABLE),
        uaddr::READ_T1 => ControlWord::new(false, false, true, false, false, false, false, false)
            .with_internal(MDR_LOAD | BUS_DATA_ENABLE),
        uaddr::READ_T2 => NONE,
        uaddr::WRITE_T0 => NONE.with_internal(MAR_LOAD | BUS_ADDRESS_ENABLE),
        uaddr::WRITE_T1 => ControlWord::new(false, false, false, true, false, false, false, false)
            .with_internal(MDR_LOAD | BUS_DATA_ENABLE),
        uaddr::WRITE_T2 => ControlWord::new(false, false, false, true, false, false, false, false)
            .with_internal(BUS_ADDRESS_ENABLE | BUS_DATA_ENABLE | ARCH_COMMIT),
        value => {
...
mod tests {
    ...
    #[test]
    fn compact_ldi_addi_rows_expose_full_latched_alu_path() {
        for opcode in [op::LDI, op::ADDI] {
            ...
            let base = execute_address(opcode).unwrap();
            let row0 = control_word_at(base, opcode);
            let row1 = control_word_at(base + 1, opcode);
            let row2 = control_word_at(base + 2, opcode);
            assert!(row0.has_internal(internal::OPERAND_A_LOAD));
